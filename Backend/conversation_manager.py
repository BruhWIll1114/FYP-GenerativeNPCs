...
class ConversationManager:
    ...
    def _invoke_timeout(
        self, fn, *args, timeout: float = LLM_TIMEOUT_SECONDS, label: str = "LLM"
    ):
    ...
    def check_conversation(
        ...
        def _run():
            try:
                if client:
                    for a_id in agent_ids:
                        partner_id = next(
                            (pid for pid in agent_ids if pid != a_id), None
                        )
        ...
        t = threading.Thread(
            target=_run, daemon=True, name=f"Conv-{'&'.join(agent_ids)}"
        )
        t.start()
    ...
    def generate_agent_response(
        ...
        result = self._invoke_timeout(
            self.graph.invoke, inputs, config, label="Dialogue generation"
        )
        if result is None:
            return ""
    ...
    def handle_conversation(
        ...
        for a_id in agent_ids:
            agent_executions[a_id]["is_chatting"] = True
            agent_executions[a_id]["is_busy_until"] = (
                time.time() + 600
            )  # Prevent other tasks while chatting
        ...
        if client and dialogue_lines:
            ...
            print("(With client) Conversation finished")
        else:
            print("(No client) Conversation finished")
            for a_id in agent_ids:
                if a_id in agent_executions:
                    agent_executions[a_id]["is_chatting"] = False
                    agent_executions[a_id]["is_busy_until"] = time.time() + 5
        ...
        commitment_manager.assign_commitment(
            dialogue_lines, agent_executions, current_time
        )

    def summarize_conversation_and_store(
        self, user_id: str, raw_log: str = None, log_id: str = None
    ) -> str:
        if self.debug_mode:
            print("(debug) summary skipped")
            return ""
        ...
        resp = self._invoke_timeout(
            self.llm.invoke,
            [system, user_msg],
            label=f"Summary generation for {user_id}",
        )
        if resp is None:
            return ""
    ...
    def check_conversation_status(self, sender_id, sender_persona, dialogue_history):
        ...
        if len(dialogue_history) % 2 == 0:
            conv_history = "\n".join(
                [f"{d['speaker']}: {d['text']}" for d in dialogue_history]
            )
            prompt_content = (
            ...
            resp = self._invoke_timeout(
                self.llm.invoke, prompt_content, label=f"Status check for {sender_id}"
            )
            if resp is None:
                return True
    ...
    def generate_dialogue(self, area: str, participants: list, context_str):
        ...
        try:
            ...
            for i in range(max_turns):
                ...
                status = self.check_conversation_status(
                    sender_id, sender_persona, dialogue_history
                )
                if status:
            ...
            self.record_conversation(
                agent_ids, dialogue_history, area, max_turns, agent_personas
            )
        except Exception as e:
            print(f"Error during conversation: {e}\n{traceback.format_exc()}")

    def record_conversation(
        self, participants, dialogue, place, max_turns=None, agent_personas=None
    ):
        if not dialogue:
            return

        if self.debug_mode:
            print("(debug) conversation log skipped")
            return
                ...
                for i in range(len(participants)):
                    for j in range(len(participants)):
                        ...
                        def _update_pref(a, b, personas, log):
                            ...
                            if score is None:
                                self.preference_manager.init_impression(
                                    a, personas[a], b, log
                                )
                            else:
                                self.preference_manager.update_impression(
                                    a, personas[a], b, score, log
                                )
        ...
        with self.db_lock:
            log_id = self.memory_manager.add_conversation_log(
                participants, log_string, place
            )
