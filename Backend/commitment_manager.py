...
_commitment_executor = ThreadPoolExecutor(
    max_workers=1, thread_name_prefix="CommitmentWorker"
)
...
class CommitmentManager:
    ...
    def get_commitments(self, agent_id: str) -> list[dict]:
        rows = self.conn.execute(
            "SELECT commitment_id, event_name, start_time, end_time FROM commitments WHERE invitee_id=?",
            (agent_id,),
        ).fetchall()
        ...
        commitment_list = [f"{n[0]}: {n[1]} from {n[2]} to {n[3]}" for n in rows]
    ...
    def pending_steps(
        self, steps: list[Step], start_time: str, end_time: str
    ) -> list[Step]:
        try:
            ...
            window_steps = [
                (t, desc) for t, desc in steps if rounded_start <= t <= rounded_end
            ]
            return window_steps
    ...
    def assign_commitment(
        self, dialogue_lines, agent_executions, current_time
    ):  # Queue commitment check when a conversation happens
        if not dialogue_lines:
            return False

        request = "\n".join(dialogue_lines)
        commitment_queue.put(
            (list(dialogue_lines), agent_executions, request, current_time)
        )
        print("[COMMITMENT] Queued invitation analysis.")
        return True

    def check_commitment(
        self, dialogue_lines, agent_executions, sim_time
    ):  # identify invitation within conversation
        dialogue_text = "\n".join(dialogue_lines)
        ...
        if response.lower() == "none":
            print("[COMMITMENT] No invitation detected in the dialogue.")
            return
        ...
        if (
            not target_id
            or not inviter_id
            or target_id not in agent_executions
            or inviter_id not in agent_executions
        ):
            print("[COMMITMENT] Invalid invitation format.")
            return

        existing_commitment = self.get_commitments(target_id)
        if existing_commitment and len(existing_commitment) >= MAXIMUM_COMMITMENTS:
            print(
                f"[COMMITMENT] Skipping commitment check for {target_id} due to already existing commitments [{MAXIMUM_COMMITMENTS}]."
            )
            return

        print(
            f"[COMMITMENT] Detected invitation: {invite_info['initiator']} -> {invite_info['invitee']} for {invite_info['event_name']} at {invite_info['start_time']}"
        )
        # self.initiator_commitment(agent_executions, target_id, invite)
        self.decide_commitment_invitee(agent_executions, invite_info)

    def decide_commitment_invitee(
        self, agent_executions: dict, invite_info: dict
    ):  # for invitee to decide whether accept the invitation, and if accept, what steps to replan within the event window
        ...
        impression_score = PreferenceManager().get_preference_score(
            invitee_id, initiator_id
        )
        relationship_type = PreferenceManager().get_relationship_type(
            invitee_id, initiator_id
        )
        ...
        if existing_commitment:
            ...
            additional_prompt = (
                "There are no existing commitments from the CommitmentManager."
            )

        if impression_score and impression_score <= 6.5:
            print(
                f"[COMMITMENT] Declined {event_name} from {initiator_id} due to low impression score ({impression_score:.1f})"
            )
            return {"state": "DECLINE", "steps": None}

        if relationship_type and relationship_type == "Stranger":
            print(
                f"[COMMITMENT] Declined {event_name} from {initiator_id} due to Stranger relationship"
            )
            return {"state": "DECLINE", "steps": None}
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            try:
                ...
                print(
                    f"[COMMITMENT] LLM response timed out for {invitee_id} on event {event_name}. Defaulting to DECLINE."
                )
                return {
                    "state": "DECLINE",
                    "steps": None,
        ...
        if state != "ACCEPT":
            print(
                f"[COMMITMENT] {invitee_id} declined the invitation for {event_name}. Reason: {decision_data.get('reason', 'No reason provided')}"
            )
            return {"state": "DECLINE", "steps": None}

        if not isinstance(new_steps, str) or not new_steps.strip():
            print(
                f"[COMMITMENT] Invalid or empty steps provided by LLM for {invitee_id} on event {event_name}. Defaulting to DECLINE."
            )
            return {"state": "DECLINE", "steps": None}
        ...
        self.build_commitment_initiator(
            initiator_persona, invite_info, initiator_steps, new_steps
        )

    def build_commitment_initiator(
        self, persona: str, invite_info: dict, steps: list[Step], new_steps: str
    ):  # after the invitee accepts the commitment, the initiator also needs to replan their schedule to fit the event in.
    ...
    def tasks_replan(self, agent_id: str, new_steps: str, steps: list[Step]) -> bool:
        ...
        success = modify_plan(
            agent_id, description=new_desc, new_emojis=original_emojis
        )

        if success:
            print(
                f"[REPLAN] Successfully updated plan for {agent_id} with new commitment."
            )
            return True
...
_commitment_thread = threading.Thread(
    target=_commitment_worker, daemon=True, name="CommitmentWorker"
)
...
def main():
    ...
    try:
        success = commitment_manager.tasks_replan(
            agent_id=agent_id, new_steps=new_steps, steps=steps
        )
