...
class UnityClient:
    ...
    def _mark_conv_finished(self, agent_id, partner_id):
        ...
        with self._conv_finished_lock:
            self._conv_finished_events.add(
                (str(agent_id), str(partner_id) if partner_id else None)
            )
    ...
    def build_and_send(
        ...
        **kwargs,
        ...
        return self.send_command(
            cmd, agent_id=agent_id, wait_for_response=wait_for_response
        )
    ...
    def move_to(
        ...
        self.build_and_send(
            "move_to", agent_id, target=target, content=content, description=description
        )
        if wait_for_response:
    ...
    def set_chatting(
        self, agent_id: str = None, content: str = None, partner_id: str = None
    ):
        kwargs = {}
        if partner_id:
            kwargs["partner"] = partner_id
        self.build_and_send(
            "set_chatting", agent_id=agent_id, content=content, **kwargs
        )

    def show_dialogue(
        self, content: str, agent_id: str = None, display_time: float = None
    ):
        self.build_and_send(
            "show_dialogue", agent_id, content=content, display_time=display_time
        )

    def interact(
        self, target: str, method: str, parameters: dict = None, agent_id: str = None
    ):
        kwargs = {"target": target, "method": method}
        if parameters:
            kwargs.update(parameters)
        return self.build_and_send(
            "interact", agent_id, wait_for_response=True, **kwargs
        )
    ...
    def handle_incoming_command(
        self, command_dict, agent_executions, interaction_manager=None
    ):
        ...
        if action == "conversation_finished":
            print(
                f"[TCP] Received conversation_finished: agent={agent_id}, partner={partner_id}"
            )
        ...
        if not session_id:
            ...
            if availability.get("status") != "success" or not availability.get(
                "available", False
            ):
                self.send_response(
                    agent_id=agent_id, request_id=request_id, payload=availability
                )
                return

            try:
                payload = interaction_manager.start_conversation(
                    agent_id,
                    question=user_text,
        ...
        try:
            payload = interaction_manager.continue_conversation(
            ...
            if session_agent in agent_executions:
                ...
                agent_executions[session_agent]["is_busy_until"] = time.time() + (
                    1 if ended else 600
                )
        except ValueError as e:
            payload = {
    ...
    def receive_msg(self, agent_id: str, timeout: float = 5.0):
        ...
        try:
            ...
            return [
                msg.decode("utf-8").strip() for msg in data.split(b"\n") if msg.strip()
            ]
        except TimeoutError:
            return
        except Exception as e:
    ...
    def wait_for_conv_finish(self, agent_id: str, timeout: float = 300.0):
        ...
        while time.time() - start_time < timeout:
            if self._consume_conv_finished(agent_id):
                print("[TCP] User finished reading. Resuming agents.")
                return True
            time.sleep(0.05)
        return False
    ...
    def check_for_incoming(
        self, agent_id: str, agent_executions: dict, interaction_manager=None
    ):
            ...
            try:
                if msg.startswith("{"):
                    ...
                    self.handle_incoming_command(
                        cmd, agent_executions, interaction_manager=interaction_manager
                    )
            except Exception as e:
