...
def check_reflect(agent_id: str, clock, agent_executions: dict, client=None) -> bool:
    ...
    worker_is_idle = not any(
        data.get("is_reflecting") for data in agent_executions.values()
    )
    if worker_is_idle:
    ...
    print(
        f"[REFLECT] {agent_id}: importance sum={total:.0f} >= {REFLECTION_THRESHOLD}. Queued for reflection."
    )
    return True
...
def _reflection_worker() -> None:
    while True:
        ...
        try:
            ...
            print(
                f"[REFLECT] Worker error for {agent_id}: {e}\n{traceback.format_exc()}"
            )
        finally:
            with _pending_lock:
...
threading.Thread(
    target=_reflection_worker, daemon=True, name="ReflectionWorker"
).start()
...
def _generate_questions(agent_id: str) -> list[str]:
    ...
    numbered = "\n".join(
        f"{i + 1}. [{r['source']}] {r['text']}" for i, r in enumerate(records)
    )
        ...
        for col in cols:
            try:
                ...
                for doc_id, doc, meta, dist in zip(
                    ...
                    final_score = (
                        (0.5 * recency) + (0.3 * importance) + (0.2 * relevance)
                    )
...
def _synthesize_and_store(agent_id: str, memories: list[dict], clock) -> list[dict]:
    ...
    try:
        resp = reflect_llm.invoke(
            [
                ...
                {
                    "role": "user",
                    "content": f"Memory statements for {agent_id}:\n{text_numbered}",
                },
            ]
        )
        data = json.loads(resp.content)
    ...
    for item in insights:
        ...
        memory_manager.save_reflection(
            user_id=agent_id,
