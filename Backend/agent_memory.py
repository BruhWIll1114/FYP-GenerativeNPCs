...
class AgentMemoryManager:
    ...
    def save_reflection(
        self, user_id: str, insight: str, importance: int, cited_memories: list
    ) -> str:
        ts = int(time.time())
        cur = self.conn.execute(
    ...
    def get_importance_score(self, user_id: str) -> float:
        ...
        obs_score = self.conn.execute(
            "SELECT COUNT(*) FROM observation WHERE user_id=? AND used=0", (user_id,)
        ).fetchone()[0]
    ...
    def mark_records_used(self, user_id: str) -> None:
        ...
        self.conn.execute(
            "UPDATE summaries SET used=1 WHERE user_id=? AND used=0", (user_id,)
        )
        self.conn.execute(
            "UPDATE observation SET used=1 WHERE user_id=? AND used=0", (user_id,)
        )
        self.conn.execute(
            "UPDATE reflection SET used=1 WHERE user_id=? AND used=0", (user_id,)
        )
...
if __name__ == "__main__":
    ...
    try:
        memory_result = AgentMemoryManager().get_memory(
            query=test_query, user_id=test_user_id, current_hours=test_current_hours
        )
