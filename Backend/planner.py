...
def get_plan(user_id: str):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT plan_json FROM plans WHERE user_id = ? ORDER BY created_on DESC LIMIT 1",
            (user_id,),
        )
        row = cur.fetchone()
...
def get_plan_id(user_id: str):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT plan_id FROM plans WHERE user_id = ? ORDER BY created_on DESC LIMIT 1",
            (user_id,),
        )
        row = cur.fetchone()
...
def modify_plan(
    user_id: str, description: str | None = None, new_emojis: list[str] | None = None
):
    conn = None
    try:
        conn = sqlite3.connect(DB_PATH)
...
def plan_prompt(
    background: str, candidate_info: list[dict], reflections: list[str]
) -> str:
...
def decompose_plan(
    ...
    duration_prompt: str,
    candidate_info: list[dict] = None,
    background: str | None = None,
    emoji_generation: bool = False,
    ...
    if reflections:
        prompt += f"- Here is some recent reflections needs to be considered for the plan generation. Informations including relationships with others, or improtance things for the person, will affect tasks that the agent want to do:\n{reflections}\n"
...
def init_plan(
    user_id: str, background: str | None, candidate_info: list[dict]
) -> dict[str, Any]:
    plan_id = str(uuid.uuid4())
    ...
    child_plan = decompose_plan(
        top_plan,
        duration_prompt="1 hour",
        candidate_info=candidate_info,
        background=background,
        emoji_generation=False,
        reflections=reflections,
        auto_store=False,
    )
    child_plan_2 = decompose_plan(
        child_plan,
        duration_prompt="30 minutes",
        candidate_info=candidate_info,
        background=background,
...
def generate_plans_for_missing_agents(
    agents_data: dict[str, Any], candidate_info: list[dict]
):
    eligible_agents = {
        uid: info for uid, info in agents_data.items() if get_plan_id(uid) is None
    }
    skipped_count = len(agents_data) - len(eligible_agents)
    print(
        f"[Planner] Found {len(eligible_agents)} agents without existing plans. Skipping {skipped_count} agents with existing plans."
    )
    return generate_plans(eligible_agents, candidate_info)


def generate_plans_for_specific_agents(
    agents_data: dict[str, Any], candidate_info: list[dict], target_agent_ids: list[str]
):
    target_set = set(target_agent_ids)
    eligible_agents = {
        uid: info for uid, info in agents_data.items() if uid in target_set
    }
...
if __name__ == "__main__":
    ...
    with open(AGENT_STATE_DIR, encoding="utf-8") as f:
    ...
    menu_options = {
        "1": {
            ...
            "function": lambda: generate_plans_for_missing_agents(
                agents, candidate_info
            ),
            "print_result": lambda generated: print(
                "Generated plans for: " + ", ".join(generated)
                if generated
                else "No plans generated."
            ),
        },
        "2": {
            ...
            "function": lambda: generate_plans_for_specific_agents(
                ...
                [
                    item.strip()
                    for item in input("Enter agent IDs (comma-separated): ").split(",")
                    if item.strip()
                ],
            ),
            "print_result": lambda generated: print(
    ...
    while True:
        try:
            menu_text = (
                ...
                + "\n".join(
                    f"{key}) {opt['description']}" for key, opt in menu_options.items()
                )
