...
from planner import generate_plans, get_plan, parse_plan
...
def agent_node(state: State, config: RunnableConfig):
    ...
    if user_msgs:
        memory_context = memory_manager.get_memory(
            user_msgs[-1]["content"], user_id, clock.get_sim_hour(), partner_id
        )

    partner_context = ""
    if partner_id:
        relationship_score = preference_manager.get_preference_score(
            agent_name, partner_id
        )
        if relationship_score is None:
            relationship_score = 3.0
        relationship_type = preference_manager.get_relationship_type(
            agent_name, partner_id
        )
        if relationship_type is None:
            relationship_type = "Stranger"
        partner_context = (
            f"You are currently talking to {partner_id}. {partner_id} has a {relationship_type} relationship with you. Use suitable pronoun, manner when talking."
...
def get_graph():
    ...
    )  # deciding whether to save/retrieve memories
    builder.add_edge(
        "tools", "agent"
    )  # accessing tools, retrieve memories back to agent
...
def find_target(action: str, agent_id: str):
    path_nodes = tree.find_suitable_location(
        action, agent_id, simulation_day=clock.get_sim_days()
    )
...
def execute_agent_action(
    ...
    # record_observation(agent_id, area_name, obj_name, action, area_state_manager=area_state_manager, memory_manager=memory_manager, clock=clock)
    ...
    with area_manager.lock:
        ...
        if (
            obj_info.get("state") == "occupied"
            and obj_info.get("occupied_by") != agent_id
        ):
        ...
        group = [
            {
                ...
                "tone": agent_executions[agent_id]["tone"],
            },
            {
                "id": partner_id,
                "persona": agent_executions[partner_id]["persona"],
                "tone": agent_executions[partner_id]["tone"],
            },
        ]
...
def main():
    ...
    try:
        for agent_id, data in agent_executions.items():
            ...
            print(
                f"[{agent_id}] Loaded plan for {agent_id} with {len(data['steps'])} steps."
            )

        simulation_active = True
        while not shutdown:
            for aid in agent_executions:
                client.check_for_incoming(
                    aid, agent_executions, interaction_manager=interaction_manager
                )
            ...
            if clock.is_new_day() and clock.get_sim_days() > 0:
                ...
                for config in agents_config:
                    ...
                    client.move_to(
                        home_node, "", "Return Home", agent_id, wait_for_response=True
                    )
            ...
            if simulation_active and clock.get_sim_hour() >= 6:
                for agent_id, data in agent_executions.items():
                    ...
                    current_desc = "\n".join(
                        [f"{i + 1}) {t}: {a}" for i, (t, a) in enumerate(data["steps"])]
                    )
                    ...
                    if (
                        not is_busy
                        and not is_cooldown
                        and not data["is_chatting"]
                        and not data["is_reflecting"]
                        and data["current_step"] < len(data["steps"])
                    ):  # has tasks remaining
                        ...
                        if (
                            "emojis" in data
                            and isinstance(data["emojis"], list)
                            and step_index < len(data["emojis"])
                        ):
