...
class ExistingMemoryAdapter:
    ...
    def get_memory(
        self, query, user_id: str, current_hours: int, partner_id: str = None
    ):
        query_text = " ".join(query) if isinstance(query, list) else str(query)
        try:
            result = self.memory.get_memory(
                query_text, user_id, current_hours, partner_id
            )
...
class InterviewStorage:
    def __init__(self, sqlite_path: str, chroma_path: str):
        ...
        self.conn = sqlite3.connect(
            self.sqlite_path, isolation_level=None, check_same_thread=False
        )
        self._ensure_schema()
    ...
    def add_conversation_log(
        self, participants: list[str], log_string: str, place: str
    ) -> str:
    ...
    def save_summary(
        self, user_id: str, summary: str, importance: int, log_id: str = None
    ):
    ...
    def summarize_conversation_and_store(
        self, user_id: str, raw_log: str, log_id: str, game_hour: int
    ):
        system = {
            "role": "system",
            "content": (
...
def choose_agent(agents_state: dict[str, dict]) -> str | None:
    if not agents_state:
        return None
