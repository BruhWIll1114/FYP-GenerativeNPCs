...
class AreaStateManager:
    ...
    def _get_conn(self):
        ...
        )  # .wal(Write-Ahead Log) is a temp log file to store changes before being applied to the main db; .shm(Shared Memory) is used for coordinating access between multiple connection to the same database.
        conn.execute("""
...
class AreaSystem:
    ...
    def reset_area(self):
                ...
                if filename.endswith(".db"):
                    ...
                    with mgr.lock:
                        with mgr._get_conn() as conn:
                            conn.execute(
                                "UPDATE objects SET state = 'empty', occupied_by = NULL"
                            )
        ...
        with open(AGENT_STATE_DIR) as f:
